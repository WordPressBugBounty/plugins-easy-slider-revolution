...
if ($lightbox_count > 0) {
    $lightgallery_id = 'lightgallery_' . esc_attr($slide_data['css_id']);

    $output .= "<div id='" . esc_attr($lightgallery_id) . "' style='display:none !important;'>\n";
    for ($i = 1; $i <= $slide_data['num_slides']; $i++) {
        // LOOP THROUGH EACH SLIDE.
        if (('IMAGE' === $slide_data['slide' . $i . '_popup_type']) && ('' !== $slide_data['slide' . $i . '_popup_image'])) {
            // SLIDE CONTAINS AN IMAGE POPUP.
            $img_url     = esc_url($slide_data['slide' . $i . '_popup_image']);
            $img_title   = esc_html($slide_data['slide' . $i . '_popup_imagetitle']);
            $slide_num   = $i + 1;
            $popup_cssid = $lightgallery_id . '_' . $slide_num;
            if ('' !== $img_title) {
                $output .= "<div class='lg_item' id='" . esc_attr($popup_cssid) . "' href='" . $img_url . "' data-sub-html='" . $img_title . "'>";
            } else {
                $output .= "<div class='lg_item' id='" . esc_attr($popup_cssid) . "' href='" . $img_url . "'>";
            }
            $output .= 'slide' . $slide_num . "</div>\n";
        }
        if (('VIDEO' === $slide_data['slide' . $i . '_popup_type']) && ('' !== $slide_data['slide' . $i . '_popup_video_id'])) {
            // SLIDE CONTAINS A VIDEO POPUP.
            $video_id   = esc_attr($slide_data['slide' . $i . '_popup_video_id']);
            $video_type = esc_attr($slide_data['slide' . $i . '_popup_video_type']);
            if ('youtube' === $video_type) {
                $video_url = esc_url('http://www.youtube.com/watch?v=' . $video_id . '&mute=0');
            } elseif ('vimeo' === $video_type) {
                $video_url = esc_url('http://vimeo.com/' . $video_id . '&muted=false');
            }
            $slide_num   = $i + 1;
            $popup_cssid = $lightgallery_id . '_' . $slide_num;

            $output .= "<div class='lg_item' id='" . esc_attr($popup_cssid) . "' data-lg-size='1280-720' data-src='" . $video_url . "'>\n";
            $output .= 'slide' . $slide_num . "</div>\n";
        }
    }
    $output .= "</div>\n";
